"""
Complete Reset and 14-Sep-2026 Khatabook Sync for do-it-services:
1. Deletes ALL transaction entries (collections, bank_deposits, ledgers, portal_adjustments, attendance, denominations).
2. Resets ALL portals and bank accounts to 0 balance and 0 opening balances.
3. Resets user virtual balances to 0 and business opening cash to 0.
4. Syncs ALL 288 Khatabook entries into retailers with opening_balance_set_on = 2026-09-14.
5. Recreates fresh Opening Balance ledger entries for all retailers.
6. Verifies that Dashboard totals match Khatabook:
   - Retailer Give: Rs 9,042,178.00 | Take: Rs 9,042,178.00
   - Portal Give: Rs 0 | Take: Rs 0
   - Total Give: Rs 9,042,178.00 | Take: Rs 9,042,178.00
"""
import os
import sys
import uuid
...
NAME_MAPPING = {
    ...
    "farhat cyber cafe reli pay": "farhat cyber cafe",
}
...
RETAILERS_14SEP = [
  ...
  {
    "name": "Rizwan Retailer Portal Reli Pay",
    "phone": "9999900018",
    "take": 1529.0,
    "give": 0.0
  },
  ...
  {
    "name": "Jafer Khan Retailer Portal Soul Pay",
    "phone": "9999900023",
    "take": 3176.0,
    "give": 0.0
  },
  ...
  {
    "name": "Abhishek retailer portal reli pay",
    "phone": "9999900069",
    "take": 2484.0,
    "give": 0.0
  },
  {
    "name": "Dheeraj Tomar Reli Pay Portal",
    "phone": "9999900070",
    "take": 418.0,
    "give": 0.0
  },
  {
    "name": "Nitish Bhati Reli Pay Portal",
    "phone": "9999900071",
    "take": 429.0,
    "give": 0.0
  },
...
def run_reset_and_sync_for_db(db_name: str):
    print("=" * 60)
    print(f"🚀 Running Reset & 14-Sep Khatabook Sync on: {db_name}")
    print("=" * 60)
    ...
    try:
        ...
        # 5. Zero out any existing retailers first
        print("\n5. Zeroing existing retailers opening balances...")
        ...
        print("\n6. Syncing 288 Khatabook entries into Retailers...")
        all_retailers = db.query(Retailer).all()
        ...
        print("\n7. Recalculating ledger balances for all retailers...")
        ...
        db.commit()
        print(f"   Opening balance ledger entries created for {len(active_retailers)} retailers.")
        ...
        seed_take = sum(r['take'] for r in RETAILERS_14SEP)
        seed_give = sum(r['give'] for r in RETAILERS_14SEP)
        ...
        print(f"KHATABOOK TARGET:  You will give: Rs {seed_give:,.2f} | You will get: Rs {seed_take:,.2f}")
        print(f"Net Balance:       Rs {total_give - total_take:,.2f}")
        print(f"Date set on all:   {TARGET_DATE}")

        give_match = abs(total_give - seed_give) < 0.01
        take_match = abs(total_take - seed_take) < 0.01

        if give_match and take_match:
            print("\nPERFECT 100% MATCH WITH KHATABOOK 14-SEP REPORT!")
        else:
            print(f"\nMISMATCH: Give diff={total_give - seed_give}, Take diff={total_take - seed_take}")
        ...
        return {
            ...
            "updated": updated,
            "created": created,
            "deleted_transactions": coll_count + dep_count
        }
...
def main():
    ...
    print(f"Target tenant databases for 14-Sep reset: {target_dbs}")
