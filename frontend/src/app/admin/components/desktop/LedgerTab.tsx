...
export default function LedgerTab({
  ...
  const bankAccountList = Array.from(new Set(
    allTransactions.map(tx => tx.bankAccount).filter(Boolean)
  )).sort();
  ...
  if (searchQuery) {
    ...
    allTransactions = allTransactions.filter(tx => 
      ...
      (tx.bankAccount && tx.bankAccount.toLowerCase().includes(q)) ||
  ...
  if (bankAccountFilter !== "all") allTransactions = allTransactions.filter(tx => tx.bankAccount === bankAccountFilter);
          ...
          <div className="flex gap-2">
            <button 
              onClick={() => {
                ...
                const partyName = partyFilter !== 'all' ? partyFilter : bankAccountFilter !== 'all' ? bankAccountFilter : 'All Parties';
