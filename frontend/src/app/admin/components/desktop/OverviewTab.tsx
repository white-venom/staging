...
export default function OverviewTab({
  ...
  const [denominationBaselines, setDenominationBaselines] = React.useState<Record<string, any>>({});
  React.useEffect(() => {
  ...
  const staffListData = React.useMemo<StaffListData[]>(() => {
    ...
    return (staffUsers || []).map((user: { id: string; name: string }) => {
      ...
      // recorded notes, every deposit subtracts its own recorded notes. The
      // backend enforces sum(denominations) === amount on every transaction,
      // so this is always exact — it doesn't need to guess which notes are
      // "still in hand" the way a greedy reconstruction would, which can
      // invent an entirely different (though dollar-equal) note mix whenever
      // a deposit doesn't remove a proportional slice of every denomination.
      // If a verified baseline exists, start from it and only sum
      // transactions at or after it; otherwise start from zero and sum
      // everything.
      ...
      if (baseline) {
        ...
        cutoffMs = toUtcMs(baseline.as_of);
      }

      allStaffCols.forEach(c => {
        if (!c.denominations || (c.created_at && toUtcMs(c.created_at) < cutoffMs)) return;
      ...
      allReceivedDeps.forEach(r => {
        if (!r.denominations || (r.created_at && toUtcMs(r.created_at) < cutoffMs)) return;
      ...
      allStaffDeps.forEach(d => {
        if (!d.denominations || (d.created_at && toUtcMs(d.created_at) < cutoffMs)) return;
  ...
  }, [staffUsers, safeCollections, safeDeposits, staffComplianceLogs, denominationBaselines]);
            ...
            <form onSubmit={handleSaveCollectionEdit} className="space-y-4">
              ...
              {!editingIsDeposit ? (
                ...
                <div className="space-y-3">
                  ...
                  {/* Denominations editor for Cash Deposits */}
                  {(selectedNewPaymentMode === "cash" || selectedNewPaymentMode === "unified") && (
                    <div className="border border-slate-200 dark:border-slate-800 rounded-sm space-y-0.5">
                      <div className="px-2 py-1.5 border-b border-slate-100 dark:border-slate-800/60">
                        <span className="text-[10px] text-slate-400 font-bold uppercase">Counting Details (Notes)</span>
                      </div>
                      {[
                    ...
                    <input autoComplete="one-time-code"
                      ...
                      className={`w-full px-3 py-2 border rounded-sm text-right font-mono tabular-nums text-xs font-bold focus:outline-none ${
                        selectedNewPaymentMode === "cash" || selectedNewPaymentMode === "unified"
                          ...
                          : "bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 focus:border-slate-500 dark:focus:border-slate-400 dark:text-white"
                      }`}
                      required
