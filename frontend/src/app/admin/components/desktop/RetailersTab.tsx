...
export default function RetailersTab({
  ...
  // View switcher: "directory" | "ledger"
  ...
  const handleOpenLedger = async (retailer: any, opts?: { skipNav?: boolean }) => {
    ...
    if (!opts?.skipNav && retailer.ledger_token) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-2.5 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-1 sm:flex-initial justify-end">
            ...
            <select
              ...
              onChange={(e) => {
                const r = retailerDirectory.find((item) => String(item.id) === e.target.value);
                if (r) handleOpenLedger(r);
              }}
      ...
      {activeView === "directory" ? (
              ...
              >
                ...
                <div className="text-right shrink-0 flex flex-col items-end gap-2">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300 dark:hover:bg-indigo-900/50 rounded-sm text-[9px] font-black uppercase tracking-wider flex items-center gap-1 border border-indigo-200/80 dark:border-indigo-800 transition-colors cursor-pointer"
                  >
        ...
        /* PARTICULAR LEDGER INLINE VIEW WITH DATE FILTER */
        <div className="rounded-sm border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xs bg-white dark:bg-slate-900">
          {loadingLedger ? (
            <div className="py-24 bg-white dark:bg-slate-900 flex items-center justify-center">
            ...
            <div className="text-center py-16 bg-white dark:bg-slate-900">
              ...
              <p className="text-xs font-bold text-slate-500">Please select a retailer from the dropdown above to view their particular ledger.</p>
            </div>
          )}
        </div>
      )}
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-sm border border-slate-200 dark:border-slate-800 text-[11px] select-none">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-black font-mono tabular-nums ${(editingRetailer.balance || 0) > 0 ? 'text-emerald-600 dark:text-emerald-500' : (editingRetailer.balance || 0) < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-500'}`}>
