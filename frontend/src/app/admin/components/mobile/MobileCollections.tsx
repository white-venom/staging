  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between px-1.5">
        ...
        <p className="text-[8px] font-black text-blue-600 uppercase bg-blue-50 dark:bg-blue-900/20 px-1.5 py-0.5 rounded-sm">
      ...
      <div className="space-y-1.5">
        {collections.map((c) => (
          <div key={c.id} className="bg-white dark:bg-slate-900 rounded-sm p-2 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 bg-emerald-500/10 text-emerald-600 rounded-sm flex items-center justify-center">
                  <ClipboardList className="w-4 h-4" />
                </div>
                <div>
                  <h4 className="text-xs font-black text-slate-800 dark:text-white font-mono tabular-nums">₹{c.totalAmount.toLocaleString()}</h4>
               ...
               <div className="flex items-center gap-1">
                 ...
                 {c.retailerName?.toLowerCase().startsWith("cms") && (
                   <button
                     ...
                     title="View Remark"
                   >
                     <ChevronDown className={`w-2.5 h-2.5 text-slate-500 transition-transform ${cmsRemarksExpanded[c.id] ? 'rotate-180' : ''}`} />
                   </button>
                 )}
               </div>
               {c.retailerName?.toLowerCase().startsWith("cms") && cmsRemarksExpanded[c.id] && (
                 <div className="p-1.5 bg-slate-50 dark:bg-slate-950/40 rounded-sm border border-slate-200/50 dark:border-slate-800 text-[8px] font-medium text-slate-600 dark:text-slate-400">
