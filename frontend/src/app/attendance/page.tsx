  ...
  const isSubmitDisabled = !uploadedImageBase64 || isLocating || !!locationError || !gpsCoords;
    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#020617] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-100 via-slate-50 to-slate-100 dark:from-slate-900 dark:via-slate-950 dark:to-black">
      ...
      <div className="sticky top-0 z-50 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800/80">
        <div className="w-full max-w-md mx-auto px-2 h-12 flex items-center justify-between">
          <button 
            ...
            className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-300 transition-colors active:scale-95 border border-slate-200/40 dark:border-slate-700/50"
          >
      ...
      <div className="w-full max-w-md mx-auto px-2 py-4">
        <div className="bg-slate-900 rounded-lg p-3 shadow-xl relative overflow-hidden text-left border border-slate-700/50">
          <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl pointer-events-none -mr-8 -mt-8"></div>

          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-white/10 rounded-lg backdrop-blur-md shadow-inner border border-white/5">
              ...
              <span className={`text-[8px] uppercase tracking-wide font-black px-2 py-1 rounded-full backdrop-blur-md border ${attendance.isCheckedIn
            ...
            {attendance.isCheckedIn ? (
              <form onSubmit={handleCheckOutSubmit} className="space-y-3">
                <div className="grid grid-cols-2 gap-2 bg-black/40 p-2.5 rounded-lg border border-white/5 backdrop-blur-md">
                  <div>
                ...
                {kmError && (
                  <p className="text-[9px] text-red-200 bg-red-950/80 px-3 py-1.5 rounded-lg border border-red-500/30 text-center font-bold">
                    {kmError}
                  </p>
                )}
                ...
                <div className="flex flex-col gap-2.5">
                  <input
                    ...
                    className="w-full px-3 py-2 bg-black/30 border border-white/10 focus:border-blue-500/50 rounded-lg focus:outline-none text-xs text-white font-bold placeholder-slate-600 transition-colors shadow-inner"
                    required
                  />

                  {/* Upload image capture element */}
                  {!uploadedImageBase64 ? (
                    <label className="flex flex-col items-center justify-center border border-dashed border-slate-700/80 hover:border-blue-500/50 bg-black/20 hover:bg-black/30 p-4 rounded-lg cursor-pointer transition-all duration-300 group">
                    ...
                    <div className="relative rounded-lg overflow-hidden border border-slate-700/50 group">
                      <img src={uploadedImageBase64} alt="Meter Preview" className="w-full h-28 object-cover" />
                      <button
                        type="button"
                        onClick={() => setUploadedImageBase64(null)}
                        className="absolute top-1 right-1 bg-red-600/90 hover:bg-red-500 text-white p-1 rounded shadow-lg text-[8px] font-black uppercase tracking-wider px-2 backdrop-blur-md border border-red-500/20 active:scale-95 transition-transform"
                      >
                        Retake
                      </button>
                      <div className="absolute bottom-1 left-1 bg-black/70 backdrop-blur-sm px-2 py-0.5 rounded text-[7.5px] font-black uppercase tracking-wider text-slate-300 border border-white/5 flex items-center gap-1">
                  ...
                  {isLocating && (
                    <div className="flex items-center justify-center gap-1.5 p-2 bg-blue-950/40 border border-blue-900/40 text-[8px] font-black uppercase tracking-wider text-blue-400 rounded-lg animate-pulse">
                  ...
                  {locationError && (
                    <div className="p-2 bg-red-950/40 border border-red-900/40 rounded-lg text-center">
                      <p className="text-[8px] text-red-400 font-black uppercase tracking-wider mb-1.5 flex items-center justify-center gap-1">
                        <span className="flex items-center gap-0.5 justify-center"><svg className="w-3 h-3 text-amber-500 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg><span>GPS Locked:</span></span> {locationError}
                      </p>
                      <button
                        type="button"
                        onClick={fetchLiveGPS}
                        className="text-[8px] font-black uppercase text-blue-400 hover:text-blue-300 tracking-wider bg-blue-950/20 border border-blue-900/30 px-2.5 py-1 rounded-md transition-all active:scale-95"
                      >
                  ...
                  {gpsCoords && (
                    <div className="flex items-center gap-1.5 p-2 bg-emerald-950/20 border border-emerald-900/30 text-[8px] font-black uppercase tracking-wider text-emerald-400 rounded-lg">
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 text-white rounded-lg text-[9px] font-black uppercase tracking-[0.2em] transition-all active:scale-[0.98] shadow-md border border-white/10 ${
                      isSubmitDisabled
              ...
              <form onSubmit={handleCheckInSubmit} className="space-y-3">
                ...
                <div className="flex flex-col gap-2.5">
                  <input
                    ...
                    className="w-full px-3 py-2 bg-black/30 border border-white/10 focus:border-blue-500/50 rounded-lg focus:outline-none text-xs text-white font-bold placeholder-slate-650 transition-colors shadow-inner"
                    required
                  />
