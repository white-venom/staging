...
export default function HistoryPage() {
  ...
  const handleShareEntry = async (entry: any) => {
    ...
    const dateFormatted = formatShareDate(entry.created_at || entry.date);
      ...
      <div className="flex-1 w-full max-w-md mx-auto px-2 py-3 flex flex-col gap-2.5 select-none pb-24">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-500 hover:text-slate-900 dark:hover:text-slate-100 rounded-sm cursor-pointer"
            >
        ...
        <div className="space-y-2">
          ...
          {filteredCollections.length === 0 ? (
            <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-sm text-center text-xs text-slate-400 dark:text-slate-500">
              <Calendar className="w-4 h-4 text-slate-500 mx-auto mb-1" />
              No matching submission records found.
            </div>
          ) : (
            <div className="divide-y divide-slate-200 dark:divide-slate-800 border-b border-slate-200 dark:border-slate-800">
              {filteredCollections.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div>
                        ...
                        {c.retailerName?.toLowerCase().startsWith("cms") && cmsRemarksExpanded[c.id] && (
                          <div className="mt-1 px-1.5 py-0.5 bg-slate-50 dark:bg-slate-950/40 rounded border border-slate-200/50 dark:border-slate-800 text-[9px] font-medium text-slate-600 dark:text-slate-400">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="text-right">
                          <span className="text-xs font-extrabold text-slate-800 dark:text-slate-200 font-mono tabular-nums block">
                        ...
                        <button
                          ...
                          className="p-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700/80 transition-colors cursor-pointer flex items-center justify-center"
                          title="Share Entry"
                        >
                    ...
                    {isExpanded && (
                      <div className="px-2 pb-2 pt-1 border-t border-slate-100 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-950/20 space-y-2 text-[10px] font-bold text-slate-500 dark:text-slate-400">
                        <div>
