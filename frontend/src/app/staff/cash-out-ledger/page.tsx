      ...
      <div className="flex-1 w-full max-w-md mx-auto px-2 py-3 flex flex-col gap-2.5 select-none pb-24">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase tracking-wider">Cash Out Ledger</h1>
              <p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold">Your historical cash deposits</p>
            </div>
          </div>
        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="text-right">
                            {d.recipient_staff_id === currentUser?.id && d.deposit_type === "staff" ? (
                              ...
                              <span className="text-[13px] font-black text-red-600 dark:text-red-500 block font-mono tabular-nums">
                                -₹{d.amount?.toLocaleString()}
                              </span>
                            )}
                          </div>
                          ...
                          {d.remarks && (
                            <div className="border-t border-slate-200/40 dark:border-slate-800/40 pt-1.5 flex items-start gap-1">
                              ...
                              <div>
                                ...
                                <span className="text-slate-700 dark:text-slate-300 font-bold text-[9.5px]">{d.remarks}</span>
                              </div>
                            </div>
                          )}

                          {/* Edit/Delete Actions */}
                          {d.staff_id === currentUser?.id && (() => {
                            const diffMinutes = d.created_at ? (new Date().getTime() - getUtcDate(d.created_at).getTime()) / 60000 : 999999;
                              ...
                              <div className="border-t border-slate-200/40 dark:border-slate-800/40 pt-2 flex items-center justify-end gap-1.5">
                                {canEdit && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      router.push(`/deposit?editId=${d.id}`);
                                    }}
      ...
      {editingItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-950/40 transition-colors">
          ...
          <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-sm w-full max-w-xs max-h-[85vh] overflow-y-auto p-4 transition-colors flex flex-col gap-3 text-slate-800 dark:text-slate-100">
            <div>
              <h3 className="text-xs font-black uppercase tracking-wider">Edit Cash Out Entry</h3>
                  ...
                  <div key={note.key} className="flex items-center justify-between text-[11px]">
                    ...
                    <div className="flex items-center gap-2.5">
                      <input autoComplete="one-time-code"
                        ...
                        min="0"
                      />
                      <span className="w-14 text-right text-slate-500 font-bold font-mono tabular-nums">₹{((editDenoms[note.key] || 0) * note.val).toLocaleString()}</span>
                    </div>
                  </div>
                ))}
