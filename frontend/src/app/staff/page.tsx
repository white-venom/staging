...
export default function StaffDashboard() {
  ...
  const { currentUser, attendance, collections, deposits, theme, toggleTheme, restoreAttendance, resetStore } = useAppStore();
  ...
  // notes. The backend enforces sum(denominations) === amount on every
  // transaction, so this is always exact — it doesn't need to guess which
  // notes are "still in hand" the way a greedy reconstruction would, which
  // can invent an entirely different (though dollar-equal) note mix whenever
  // a deposit doesn't remove a proportional slice of every denomination (e.g.
  // depositing only the big notes and leaving small notes/coins behind). If a
  // verified baseline exists, start from it and only sum transactions at or
  // after it; otherwise start from zero and sum everything.
  ...
  if (denominationBaseline) {
    ...
    cutoffMs = getUtcDate(denominationBaseline.as_of).getTime();
  }

  collections.forEach((c) => {
    if (!c.denominations) return;
    if (c.created_at && getUtcDate(c.created_at).getTime() < cutoffMs) return;
