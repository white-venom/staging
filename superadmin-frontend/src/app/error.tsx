...
export default function ErrorPage({ error, reset }: ErrorProps) {
  ...
  useEffect(() => {
    setMounted(true);
    console.error("Superadmin critical runtime error:", error);
  }, [error]);
      ...
      <div className="w-full max-w-[450px] relative z-10 flex flex-col gap-6 animate-scale-in">
        ...
        <div className="bg-white/80 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_40px_100px_-20px_rgba(15,23,42,0.08)] p-10 pt-24 relative border border-slate-200/80 overflow-visible text-center">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-36 h-36 flex items-center justify-center pointer-events-none">
            {/* Glowing Aura */}
            <div className="absolute inset-0 bg-red-600/10 rounded-full blur-2xl animate-pulse"></div>
          ...
          <div className="mb-6 space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 border border-red-200 text-red-700 rounded-full text-[9px] font-black uppercase tracking-wider mb-2">
              ...
              <span>CRITICAL ADMIN FAULT</span>
            </div>
            <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Console Exception</h2>
            <p className="text-xs text-slate-500 font-bold leading-relaxed max-w-[300px] mx-auto">
              An unexpected error occurred in the superadmin orchestration panel. System states and databases are unaffected.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 8H18.5"></path>
              </svg>
          ...
          <div className="mt-6 border-t border-slate-100 pt-4 text-left select-text">
            ...
            {showDetails && (
              <div className="mt-3 p-4 bg-slate-50 border border-slate-100 rounded-xl max-h-[150px] overflow-auto text-[10px] font-mono text-slate-600 leading-normal break-all">
                <p className="font-bold text-red-650 mb-1">Message: {error.message || "Unknown admin-portal exception"}</p>
        ...
        <div className="text-center opacity-65 space-y-1">
          <p className="text-[9px] font-black tracking-widest text-slate-500 uppercase">CrediiFlow Superadmin Console v3.0</p>
          <p className="text-[8px] font-bold text-slate-400 uppercase">Control & Orchestration Layer</p>
        </div>
