...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      const data = await superAdminApi.forgotPassword(resetEmail);
      setResetInfo(data.message || "If that email is registered, a verification code has been sent.");
      setMode("forgot-reset");
    } catch (err: any) {
      setResetError(toUserMessage(err));
    } finally {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setResetLoading(true);
    try {
      await superAdminApi.resetPassword({ email: resetEmail, otp: resetOtp, new_password: resetNewPassword });
      setMode("forgot-done");
    } catch (err: any) {
      setResetError(toUserMessage(err));
    } finally {
  ...
  const inputCls = "w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 focus:border-violet-500 rounded-sm text-slate-200 placeholder-slate-600 focus:outline-none transition-colors text-sm";
  const primaryBtnCls = "w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] font-black uppercase tracking-widest rounded-sm transition-colors disabled:opacity-50 disabled:pointer-events-none cursor-pointer";
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 font-sans">
      <div className="w-full max-w-md p-8 mx-4 bg-slate-900 border border-slate-800 rounded-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <button
                  ...
                  className="mt-2 text-[10px] font-bold text-violet-400 hover:text-violet-300 transition-colors cursor-pointer"
                >
                  Forgot Password?
                </button>
              </div>
            ...
            <form onSubmit={handleSendOtp} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={inputCls}
                  placeholder="you@crediiflow.in"
                />
              </div>
              <button type="submit" disabled={resetLoading} className={primaryBtnCls}>
                {resetLoading ? "Sending..." : "Send Verification Code"}
              </button>
              <button
                type="button"
                onClick={resetToLogin}
                className="w-full py-2 text-[10px] font-bold text-slate-400 hover:text-slate-200 transition-colors cursor-pointer"
              >
                Back to Sign In
              </button>
            </form>
        ...
        {mode === "forgot-reset" && (
          <>
            {resetInfo && (
              <div className="mb-6 p-3 text-[11px] bg-emerald-950/30 border border-emerald-800/50 text-emerald-400 rounded-sm text-center font-bold">
                {resetInfo}
              </div>
            )}
            {resetError && (
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="••••••••"
                />
                <p className="text-[9px] text-slate-500 font-bold mt-1.5">At least 8 characters, with a letter and a digit.</p>
              </div>
              <button type="submit" disabled={resetLoading} className={primaryBtnCls}>
        ...
        {mode === "forgot-done" && (
          <div className="text-center space-y-5">
            <div className="p-4 bg-emerald-950/30 border border-emerald-800/50 text-emerald-400 rounded-sm text-[11px] font-bold">
              Password reset successfully. All previous sessions have been signed out.
            </div>
            <button onClick={resetToLogin} className={primaryBtnCls}>
              Back to Sign In
            </button>
          </div>
        )}
